...
const SearchDisplay = () => {
  ...
  useEffect(() => {
    let config = {
      ...
      url: `https://divine-drapes.onrender.com/product/productName/${selected[0]}`,
      headers: {
    ...
    async function makeRequest() {
      ...
      try {
        ...
        setProducts(response.data.productName);
      } catch (error) {
        console.log(error);
      } finally {
        setLoading(false);
              ...
              <Flex justifyContent={"space-between"} mb="3%">
                <Stack mt="4" spacing="">
                  ...
                  <Text fontSize="sm" fontWeight={550}>
                    Customizable:{" "}
                    {prod.photo.isCust ? <span>Photo</span> : <span></span>}
                    {prod.text.isCust ? <span>,Text</span> : <span></span>}
                  </Text>
                </Stack>
